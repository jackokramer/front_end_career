$(document).ready(() => {
    ...
    const api_call = () => {
        ...
        if(location){
            $.ajax({url, type: "GET", dataType: "jsonp", success: ({main, name, sys, weather}) => {
                const weatherIcon = `http://openweathermap.org/img/wn/${weather[0].icon}@2x.png`
                weatherData.show()
                weatherData.html(`          <h3 class="city-name">${name}, ${sys.country}</h3>
              ...
              `)
              const weatherDetailsButon = $(".weather-details.button")
              weatherDetailsButon.click(() => {
                  weatherData.html(`          <h3 class="city-name">${name}, ${sys.country}</h3>
                  <p class="weather-temperature">${main.temp}°C</p>
                  <img class="weather-icon" src=${weatherIcon} alt="WeatherIcon"/>
